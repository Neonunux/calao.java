...
public class SmartBar extends JPanel implements ActionListener, ChangeListener 
{
	...
	public SmartBar (Dimension d, ResourceBundle b, Font f, Preferences p, boolean inline, boolean earTraining, boolean training)
	{
		appBundle = b;
		appFont = f;
		appPrefs = p;
		...
		if (inline == false)
		{
			...
			refreshBtn.setButtonImage(new ImageIcon(getClass().getResource("refresh.png")).getImage());
		...
		if (inline == false)
		{
			...
			listenBtn.setButtonImage(new ImageIcon(getClass().getResource("listen.png")).getImage());
		}
