...
public class GnuLecture extends JFrame implements ActionListener 
{
	 ...
	 public GnuLecture() 
	 {
		 ...
		 if ("he".equals(language) || "hu".equals(language) || "ru".equals(language) || "eo".equals(language))
		 {
			try 
			{
				bundle = new PropertyResourceBundle(new InputStreamReader(getClass().getResourceAsStream("gnupiano.lang.language_" + language + ".properties"), "UTF-8"));
			} 
			catch (IOException e) 
			{ 
			...
			bundle = ResourceBundle.getBundle("gnupiano.lang.language", new Locale(language));
		}
         ...
         try 
         {
        	InputStream fInput = getClass().getResourceAsStream("MusiSyncForScoreDate.ttf");
	 ...
	 private void updateLanguage()
	 {
		if ("iw".equals(language) || "he".equals(language) || "hu".equals(language) || "ru".equals(language) || "eo".equals(language))
		{
			try 
			{
				bundle = new PropertyResourceBundle(new InputStreamReader(getClass().getResourceAsStream("lang/language_" + language + ".properties"), "UTF-8"));
			} 
			catch (IOException e) 
			{ 
				System.out.println("Cannot load UTF8 language: " + language);
				language = "en";
				bundle = ResourceBundle.getBundle("lang/language", new Locale(language));
			}                
		}
		else
			bundle = ResourceBundle.getBundle("language", new Locale(language));
