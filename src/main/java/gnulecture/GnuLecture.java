...
public class GnuLecture extends JFrame implements ActionListener {
	...
	private static final Logger logger = LogManager.getLogger(GnuLecture.class
			.getName());
	...
	public GnuLecture() {
		...
		if (language == "") {
			...
			logger.debug("Got system language: " + locale.getLanguage());
		...
		if ("he".equals(language) || "hu".equals(language)
			...
			try {
				bundle = new PropertyResourceBundle(new InputStreamReader(
						getClass().getResourceAsStream(
								"gnulecture.lang.language_" + language
										+ ".properties"), "UTF-8"));
			} catch (IOException e) {
			...
			bundle = ResourceBundle.getBundle("gnulecture.lang.language",
					new Locale(language));
		}
		...
		if (midiDev != null) {
			...
			try {
				...
				System.out
						.println("Unable to connect the device's Transmitter to the Receiver:");
				logger.debug(e);
				midiDev.close();
			}
		}
		...
		addComponentListener(new java.awt.event.ComponentAdapter() {
			public void componentResized(ComponentEvent e) {
				logger.debug("Score Date has been resized !");
				/*
		...
		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				logger.debug("Score Date is going to be closed !");
				if (midiDev != null)
					midiDev.close();
				midiDev = null;
				if (midiControl != null)
			...
			midiOptions.addPropertyChangeListener(new PropertyChangeListener() {
				public void propertyChange(PropertyChangeEvent evt) {
					if (evt.getPropertyName() == "newMidiDevice") {
						System.out
								.println("Going to reconfigure MIDI system...");
						if (midiControl != null)
							midiControl.close();
						midiControl = null;
						if (midiDev != null)
