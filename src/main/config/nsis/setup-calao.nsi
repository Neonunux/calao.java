...
!define MUI_ICON "${RESOURCES_DIR}\${LOWERNAME}.ico"
!define MUI_UNICON "${RESOURCES_DIR}\${LOWERNAME}_uninstall.ico"
...
!define MUI_FINISHPAGE_LINK http://${LOWERNAME}.colombbus.org
!define MUI_FINISHPAGE_LINK_LOCATION http://${LOWERNAME}.colombbus.org
# Ne marche pas ??
...
LangString UNINSTALL_LINK ${LANG_ENGLISH} "Uninstall $(^Name)"
LangString UNINSTALL_LINK ${LANG_FRENCH} "Désinstaller $(^Name)"
LangString UNINSTALL_LINK ${LANG_SPANISH} "Desinstalar $(^Name)"
LangString FINISH_CONTENT ${LANG_ENGLISH} "To start $(^Name), click on the Start menu -> $(^Name).$\r$\nYou can also click on the shortcut $(^Name) on the desktop.$\r$\nFor more information, you can go on our website : http://${LOWERNAME}.colombbus.org"
LangString FINISH_CONTENT ${LANG_FRENCH} "Pour lancer $(^Name), cliquez sur $(^Name) dans le menu Démarrer -> $(^Name).$\r$\nVous pouvez aussi cliquer sur le raccourci $(^Name) sur le bureau.$\r$\nPour d'autres informations, vous pouvez aller sur notre site http://${LOWERNAME}.colombbus.org"
LangString FINISH_CONTENT ${LANG_SPANISH} "Para iniciar $(^Name), clic en el menú Inicio -> $(^Name).$\r$\nUsted tambi�n puede hacer clic en el atajo $(^Name) en el escritorio.$\r$\nSi desea obtener más informaciones, usted puede ir a nuestro sitio web http://${LOWERNAME}.colombbus.org"
...
LangString BRANDING_TEXT ${LANG_ENGLISH} "$(^Name) Setup"
LangString BRANDING_TEXT ${LANG_FRENCH} "Installation de $(^Name)"
LangString BRANDING_TEXT ${LANG_SPANISH} "Instalación de $(^Name)"
...
LangString LICENSE_TOP ${LANG_SPANISH} "Para instalar $(^Name), debe leer y aceptar las condiciones de la licencia:"
...
OutFile "${DIST_DIR}\${LOWERNAME}_${VERSION_NUMBER}.exe"
...
ReadRegStr $2 HKLM "SOFTWARE\JavaSoft\Java Runtime Environment\$1" "JavaHome"
         ...
         MessageBox MB_YESNO|MB_ICONQUESTION "Il n'y a aucun environnement JAVA sur votre ordinateur. Avant d'installer $(^Name), il est recommandé d'en installer un (que vous pouvez trouver sur www.java.com). Etes vous sûr de vouloir continuer ?" IDYES +5
         Abort
         NoAbortFr:
...
Section Files
	...
	SetOutPath $TEMP\${LOWERNAME}_installer
	CopyFiles /SILENT /FILESONLY $INSTDIR\*.properties $TEMP\${LOWERNAME}_installer
	...
	CopyFiles /SILENT /FILESONLY $TEMP\${LOWERNAME}_installer\*.properties $INSTDIR

	# Remove temp directory
    RMDir /r /REBOOTOK $TEMP\${LOWERNAME}_installer
...
Section Registry
    ...
    WriteRegStr HKCR "${REGKEY_LAUNCH}\shell\open\command" "" '"$INSTDIR\${LOWERNAME}.exe" "%1"'
...
Section Shortcuts
    ...
    CreateShortcut "$SMPROGRAMS\$StartMenuGroup\$(^Name).lnk" $INSTDIR\${LOWERNAME}.exe
    CreateShortcut "$SMPROGRAMS\$StartMenuGroup\$(UNINSTALL_LINK).lnk" $INSTDIR\uninstall.exe
    !insertmacro MUI_STARTMENU_WRITE_END
    CreateDirectory "$DESKTOP"
    CreateShortCut "$DESKTOP\$(^Name).lnk" "$INSTDIR\${LOWERNAME}.exe"
    #CreateDirectory "{userappdata}\Microsoft\Internet Explorer\Quick Launch"
    #CreateShortCut "{userappdata}\Microsoft\Internet Explorer\Quick Launch\$(^Name).lnk" "$INSTDIR\${LOWERNAME}.exe"
